...
{
    global $allText,$service;
    $service = new Google_Service_Docs($client);
    $doc = $service->documents->get($fileId);

    //$doc = $driveService->documents->get($fileId);
...
function getReplaceLinks($str, $startDelimiter, $endDelimiter, $isUser)
{
    ...
    while (false !== ($contentStart = strpos($str, $startDelimiter, $startFrom))) {
        ...
        $str = replaceLinks($replace, $str, $mp4, $mp3, $youtube, $isUser);
...
function replaceLinks($replace, $str, $mp4, $mp3, $youtube, $isUser)
{
    $mp4Delimiter = '';
    $mp3Delimiter = '';
    $youtubeDelimiter = '';
    ...
    if (strpos($replace, 'youtu.be') !== false) {
...
function insertToArchive($docName, $dbc, $reimport, $doc_id, $content)
{
    ...
    if ($reimport == 1) {
        $query_update_document = 'UPDATE  moodle_doc_db.documents  SET doc_name= "' . $docName . '", content="' . mysqli_real_escape_string($dbc, $content) . '", document_url ="' . $url . '"
    WHERE id=' . $doc_id;
...
,content=values(content),sort_id=values(sort_id),type =values(type),bUpdate =values(bUpdate),bChanged =values(bChanged),lesson_id =values(lesson_id),uppercss =values(uppercss),section_id =values(section_id),lowercss =values(lowercss),binsert =values(binsert),section_id =values(section_id),charVal =values(charVal),level_id =values(level_id),bDelete =values(bDelete),toUpdate = values(toUpdate)';
        ...
        $query_insert_document = 'INSERT INTO moodle_doc_db.documents (doc_name, content,document_url) VALUES ("' . $docName . '","' . mysqli_real_escape_string($dbc, $content) . '", "' . $url . '")
    ON DUPLICATE KEY UPDATE content=values(content)';
        ...
        if ($documentId) {
            ...
            $query = 'INSERT INTO moodle_doc_db.course_server (document_id,server_id) VALUES (' . $documentId . ',' . $server . ')
    ...
    if (strpos($key, '</h5>') === false) {
        if (strpos($key, '</h6>') === false) {
            ...
            if (!trim($chapter_nums) && trim($chapter) && strpos($chapter, 'Table of content') === false) {
                ...
                checkSection($id, $chapter,$section_id);
                $checkSections = 1;
    ...
    if (trim($chapter_nums)) {
        ...
        if ($section) {
            ...
            if (strpos($key, '</h1>') !== false) {
                ...
                $parent_id = 0;
                $counter_l1++;
                $query_insert_toc = "INSERT INTO moodle_doc_db.toc (doc_id,sort_id,doc_name, date_time, chapter_id, chapter, parent_id,content,type,charVal,level_id,uppercss,lowercss,section_id,toUpdate) 
            ...
            ON DUPLICATE KEY UPDATE level_id=values(level_id),date_time=values(date_time),parent_id=values(parent_id),chapter=values(chapter),content=values(content),type =values(type),bUpdate =values(bUpdate),bChanged =values(bChanged),uppercss =values(uppercss),lowercss =values(lowercss),binsert =values(binsert),charVal =values(charVal),section_id =values(section_id),bDelete =values(bDelete),toUpdate =" . $uStatus;
                ...
                $section_id = $counter_l1.".".$counter_l2;
                $counter_l3 = 0;
                $qid1 = $parent_id1;
            ...
            ON DUPLICATE KEY UPDATE level_id=values(level_id),date_time=values(date_time),parent_id=values(parent_id),chapter=values(chapter),content=values(content),type =values(type),bUpdate =values(bUpdate),bChanged =values(bChanged),uppercss =values(uppercss),lowercss =values(lowercss),binsert =values(binsert),charVal =values(charVal),section_id =values(section_id),bDelete =values(bDelete),toUpdate =" . $uStatus;
                ...
                $section_id = $counter_l1.".".$counter_l2.".".$counter_l3;
                $counter_l4 = 0;
                $qid1 = $parent_id2;
            ...
            ON DUPLICATE KEY UPDATE level_id=values(level_id),date_time=values(date_time),parent_id=values(parent_id),chapter=values(chapter),content=values(content),type =values(type),bUpdate =values(bUpdate),bChanged =values(bChanged),uppercss =values(uppercss),lowercss =values(lowercss),binsert =values(binsert),charVal =values(charVal),section_id =values(section_id),bDelete =values(bDelete),toUpdate =" . $uStatus;
                ...
                $section_id = $counter_l1.".".$counter_l2.".".$counter_l3.".".$counter_l4;
...
function checkSection($id, $chapter,$section)
{
    global $reimport, $allText,$service,$fileId;
    ...
    if ($reimport) {
        ...
        foreach ($allText as $currText) {
            ...
            if (preg_match_all("/(.*?)(".trim($chapter).")(.*?)/", $currText, $matches, PREG_SET_ORDER)) {
                ...
                foreach ($matches as $match) {
                    $modifiedText = str_replace($match[0], $section." ".trim($chapter), $modifiedText);
                }
...
// you could dump out $forEasyCompare to see the changes that would be made
        ...
        $response = $service->documents->batchUpdate($fileId, $batchUpdateRequest);
echo $response;
if(!$response){
    $response = [
            'response' => false,
            'text' => $response . ' :=> Problem occurred',
        ];
    ...
    foreach ($tags as $tag) {
        ...
        foreach ($elementList as $element) {
            ...
            if ($element->textContent != '') {
                ...
                $query_insert_toc = "INSERT INTO nts_site.project_course_choices (question_id,text,score,response) 
...
function addQuestions($sort_id, $chapter_nums, $chapter, $contentPerChapter, $qtype, $Page_id)
{
    ...
    $query_insert_toc = "INSERT INTO nts_site.project_course_question (title,text,type)
VALUES ('" . $chapter_nums . $chapter . "','" . mysqli_real_escape_string($dbc, $contentPerChapter) . "'," . $qtype . ")
  ON DUPLICATE KEY UPDATE text=values(text)";
    ...
    if ($qid) {
        $query_insert_toc = "INSERT INTO nts_site.project_course_question_to_page (question_id,page_id,sort_id)
VALUES (" . $qid . "," . $Page_id . "," . $sort_id . ")";

        mysqli_query($dbc, $query_insert_toc) or die(mysqli_error($dbc));

        $update_toc = "UPDATE moodle_doc_db.toc SET hasQuestion = 1 where id=" . $Page_id;
...
//print_r($archive_inserts);
    ...
    foreach ($difference as $key => $value) {
        ...
        $query = "UPDATE   toc a SET a.bUpdate=1 WHERE chapter_id = '" . $key . "'";
