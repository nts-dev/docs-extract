...
switch ($action) {
    ...
    case IMPORTURL:
        ...
        if ($docName != "")
            //
            $value = getContent($client, $fileId, $docName);
...
//            $check = readGoogleDocUrl($content);
           ...
           // echo json_encode(array('response' => false, 'server' => $server, 'text' => 'Problem Occurred while extracting your document!'));
        }
...
//get content
function getContent($client, $fileId, $docName)
{
    try {
    ...
    $response = $driveService->files->export($fileId, 'application/zip', array(
        'alt' => 'media'));
    $path = $_SERVER["DOCUMENT_ROOT"] . "/docs_extract_new/documentFiles/";
    if (!file_exists($path)) {
        if (!mkdir($path, 0774) && !is_dir($path)) {
            throw new \RuntimeException(sprintf('Directory "%s" was not created', $path));
        }
    }
    $myFile = $path . $docName . ".zip";
    $docFolder = "/docs_extract_new/documentFiles/" . $docName;
    $outHandle = fopen($myFile, "w+");
    $file = fwrite($outHandle, $response->getBody());
    $value = 0;
    fclose($outHandle);
    if ($file) {
        $myPath = $path . $docName;
      //when updating the course, first remove previous folder
        if( $reimport){
            xrmdir($myPath);
        }
        ...
        if (!file_exists($myPath)) {
            if (!mkdir($myPath, 0755) && !is_dir($myPath)) {
                throw new \RuntimeException(sprintf('Directory "%s" was not created', $myPath));
          }
        }
        else{
            xrmdir($myPath);
        ...
        $x = $zip->open($myFile); // open the zip file to extract
        if ($x === true) {
            $zip->extractTo($myPath); // place in the directory with same name
           readGoogleDocHtml($myPath, $docFolder, true);
            $value = 1;
            $zip->close($myFile);

//          if(chmod("../documentFiles/".$docName.".zip", 0755)) {
//              if (unlink("../documentFiles/".$docName.".zip")) {
//              }
//          }
        ...
        if ($response == 403)
            echo json_encode(
                array('response' => false,
                    'text' => 'This file is too large to be exported, Kindly download your document as zip and upload'));
...
//            </head>', $contents);
        ...
        $content = str_replace("images/image", "http://" . $_SERVER["HTTP_HOST"] . $docFolder . "/images/image", $contents);
...
function getHeadingsWithContents($bodyContent, $content)
{
    ...
    try {
        ...
        foreach ($bodyContent as $bodyObj) {

            if (is_object($bodyObj)) {

                if ($count < $size) {

                    $heading = strip_tags($bodyObj->heading);
                    $heading = str_replace('&nbsp;', '', $heading);
                    $heading = preg_replace('/\s+/', ' ', $heading);

                    $contentPerChapter = $bodyObj->content;
                    list($chapter_id, $chapter_name) = getHeadlineInformations($heading);
                    $string_lenghth = strip_tags($contentPerChapter);
                    $check = tableOfContents($bodyObj->heading, $chapter_id, $chapter_name, $contentPerChapter, $string_lenghth, $upperCss, $lowerCss, $documentId);
                } else {
