...
// Make the connection:
$dbc = mysqli_connect('83.98.243.187', 'root', 'kenya1234', 'nts_site');
...
switch ($action) {
    ...
    case 1:
        ...
        $page_id = shell_exec('moosh -n activity-add --name="' . $section_name . '" --section=' . $section_id . ' --options="--content=' . $section_name . '" page ' . $course_id);
    ...
    case 2:
        ...
        $page_id = shell_exec('moosh -n activity-add --name="' . $section_id . '" --section=' . $section_id . ' --options="--content=Long content description" page 12');
    ...
    case 3:
        ...
        $updateResult = shell_exec('moosh -n activity-config-set activity ' . $id . ' page name "' . $fieldvalue . '"');
    ...
    case 4:
        ...
        $updateResult = shell_exec('moosh -n activity-config-set activity ' . $id . ' page content "' . $fieldvalue . '"');
    ...
    case 5:
        ...
        $lesson_id = shell_exec('moosh -n activity-add --name="' . $section_name . '" --section=' . $section_id . ' --options="--title=' . $intro . '" lesson ' . $course_id);
...
//        $success = $mform->save_file('userfile', $srcFile, true);
        ...
        $response = shell_exec('moosh file-upload --filepath=backup --contextid=579 -f course image1.png');
        ...
        $response = shell_exec('moosh -n course-backup -f my_backups/"' . $filename . '".mbz ' . $course_id);
    ...
    case 7:
        ...
        $response = shell_exec('moosh course-restore --overwrite my_backups/"' . $course_name . '" ' . $course_id);
    ...
    case 8:
        ...
        $response = shell_exec('moosh activity-delete' . $module_id);
    ...
    case 9:
        ...
        $response = shell_exec('moosh -n activity-move -s ' . $section_id . ' ' . $module_id . ' ' . $module_id_to);
        // $response = shell_exec('moosh activity-move 15942 ' );
        shell_exec('moosh -n cache-course-rebuild ' . $course_id);
    ...
    case 10:
        ...
        $section_id = filter_input(INPUT_POST, 'section', FILTER_SANITIZE_NUMBER_INT);
        $course_id = filter_input(INPUT_POST, 'course_id', FILTER_SANITIZE_NUMBER_INT);
        $response = shell_exec('moosh file-upload image.png');
        // $response = shell_exec('moosh activity-move 15942 ' );
        //shell_exec('moosh -n cache-course-rebuild ' . $course_id);
        if ($response) {
            $data['data'] = array('response' => true, 'text' => $response);
    ...
    case 11:
        ...
        if($isPage) {
            $module_id = $DB->get_field('course_modules', 'id', ['instance' => $page_id, 'module' => 15], $strictness = IGNORE_MISSING);
        }
    ...
    case 12:
        ...
        $course_id = filter_input(INPUT_POST, 'course', FILTER_SANITIZE_NUMBER_INT);
        $response = shell_exec('moosh course-delete ' . $course_id);

        if ($response) {
            $data['data'] = array('response' => true, 'text' => 'Course Deleted!');
        } else {
            $data['data'] = array('response' => false, 'text' => 'Problem Occured!');
        }
        echo json_encode($data);
